...
exports.load = function (dir, deviceFamilies, logger, callback) {
	...
	afs.visitFiles(dir, function (file, full) {
		...
		var results = {},
			...
			// [TODO] Remove --------------------------------------------------------
			// Code to manage font jss properties (bug fix @see https://jira.appcelerator.org/browse/TIMOB-12264)
			// @see http://docs.appcelerator.com/titanium/3.0/#!/api/Font for font properties
				'font-weight': 'font',
				'fontWeight':  'font',
				'fontSize':    'font',
				'font-size':   'font',
						...
						rule.selectors.forEach(function (selector) {
							...
							rule.declarations.forEach(function (decl) {
								...
								if (density) {
									...
									// [TODO] Remove --------------------------------------------
									// Path to manage font jss properties (bug fix @see https://jira.appcelerator.org/browse/TIMOB-12264)
									if (fontPropMap[decl.property]) {
										results[dest][name][density][selector].hasOwnProperty(prop) || (results[dest][name][density][selector][prop] = {});
										results[dest][name][density][selector][prop][fontPropMap[decl.property] || decl.property] = stripQuotes(decl.value);
									} else {
										results[dest][name][density][selector][prop] = stripQuotes(decl.value); // keep this line only
									}
									// ----------------------------------------------------------
								} else {
									...
									// Path to manage font jss properties (bug fix @see https://jira.appcelerator.org/browse/TIMOB-12264)
									if (fontPropMap[decl.property]) {
										results[dest][name][selector].hasOwnProperty(prop) || (results[dest][name][selector][prop] = {});
										results[dest][name][selector][prop][fontPropMap[decl.property] || decl.property] = stripQuotes(decl.value);
									} else {
										results[dest][name][selector][prop] = stripQuotes(decl.value); // keep this line only
									}
									// ----------------------------------------------------------
								}
							});
